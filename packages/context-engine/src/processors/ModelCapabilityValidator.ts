...
import type { PipelineContext, ProcessorOptions, ModelCapabilities } from '../types';
...
export class ModelCapabilityValidator extends BaseProcessor {
  ...
  protected async doProcess(context: PipelineContext): Promise<PipelineContext> {
    ...
    if (validationResult.issues.length > 0) {
      ...
      if (this.options.abortOnUnsupported && !this.options.autoFix) {
        return this.abort(clonedContext, `Model does not support required features: ${validationResult.issues.join(', ')}`);
      }
   ...
   * 设置是否在不支持时中止
   */
  setAbortOnUnsupported(abort: boolean): this {
    this.options.abortOnUnsupported = abort;
    return this;
  }

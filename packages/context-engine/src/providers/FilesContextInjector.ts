...
import type { PipelineContext, ProcessorOptions, FileContext } from '../types';
...
export class FilesContextInjector extends BaseProvider {
  ...
  protected async doProcess(context: PipelineContext): Promise<PipelineContext> {
    ...
    clonedContext.messages = clonedContext.messages.map(message => {
      ...
      try {
        ...
        if (fileContext.trim()) {
          ...
          // 将文件上下文添加到消息内容中
          const updatedContent = this.mergeContentWithFileContext(message.content, fileContext);
          
          log(`为消息 ${message.id} 添加了文件上下文`);
        ...
        log.extend('error')(`处理消息 ${message.id} 文件上下文时出错: ${error}`);
        return message;
      }
    });
    ...
    log(`文件上下文处理完成，处理了 ${processedCount} 条消息，${totalFilesProcessed} 个文件`);
   ...
   * 设置文件上下文选项
   */
  setOptions(options: typeof this.fileContextOptions): this {
    this.fileContextOptions = { ...this.fileContextOptions, ...options };
    return this;
  }
   ...
   */
  getOptions() {
    return { ...this.fileContextOptions };
  }
  ...
  private preprocessFileList(fileList: string[] = []): string[] {
    return fileList.filter(fileId => {
      if (!fileId || typeof fileId !== 'string') {
        log.extend('warn')(`跳过无效的文件ID: ${fileId}`);
        return false;
      }

...
describe('selectOptimalWeightDtype', () => {
  ...
  it('should handle community models correctly', () => {
    ...
    expect(selectOptimalWeightDtype('RealFlux_1.0b_Schnell_Transformer.safetensors')).toBe('default');
    expect(selectOptimalWeightDtype('Jib_Mix_Flux_Krea_b_fp8_00001_.safetensors')).toBe('default');
    expect(selectOptimalWeightDtype('vision_realistic_flux_dev_fp8_no_clip_v2.safetensors')).toBe('default');
  });

...
<Steps>
  ...
  **支持的插件 / 扩展**：
  ...
  - **ComfyUI-Manager**：支持通过环境变量设置认证
  - **comfyui-workspace-manager**：可配置用户权限
  - **ComfyUI-Custom-Scripts**：支持认证脚本
  ...
  **支持的插件 / 扩展**：
  ...
  - **ComfyUI-API-Manager**：专门的 API 管理插件
  - **ComfyUI-Auth**：提供 Token 认证功能
  - **comfyui-security**：安全增强插件

  **获取 Token 方法**：

  1. **使用 ComfyUI-Auth 插件**：

  ```bash
  # 安装插件
  cd ComfyUI/custom_nodes
  git clone https://github.com/your-repo/ComfyUI-Auth.git
  cd ComfyUI-Auth
  pip install -r requirements.txt
  ...
  **支持的插件 / 扩展**：
  ...
  - **ComfyUI-Enterprise-Auth**：企业级认证集成
  - **comfyui-custom-headers**：自定义请求头管理
  - **ComfyUI-SSO**：单点登录集成
  ...
  **401 Unauthorized**：
  ...
  - **解决**：
    ```bash
    # 检查认证配置
    curl -v http://your-server:8188/system_stats
  ...
  **使用 FLUX Schnell（快速生成）**：
  ...
  ```
  请生成一张图片：一只可爱的橘猫坐在阳光明媚的窗台上
  ```

  **使用 FLUX Dev（高质量生成）**：

  ```
  请用高质量模式生成：夕阳下的城市天际线，赛博朋克风格，4K 高清
  ```
...
```
一位年轻女性，长发飘逸，穿着优雅的蓝色连衣裙，站在樱花盛开的公园里，
阳光从树叶间洒下，营造温暖的氛围，电影般的光线，4K 高清，细节丰富
```
